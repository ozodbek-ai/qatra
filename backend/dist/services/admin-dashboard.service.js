...
export const getDashboardOverview = async () => {
    const overview = await adminDashboardRepository.getOverview();
    const latestStudents = await adminDashboardRepository.getLatestStudents();
    let averageScore = 0;
    if (overview.quizAttempts.length > 0) {
        const totalPercentage = overview.quizAttempts.reduce((sum, item) => {
            if (item.total === 0) {
                return sum;
            }
            return (sum +
        ...
        averageScore = Math.round(totalPercentage /
    ...
    return {
        ...
        quiz: {
            attempts: overview.quizAttempts.length,
            averageScore,
        },
        latestStudents,
    };
