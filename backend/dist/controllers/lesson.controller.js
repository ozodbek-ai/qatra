...
export const getLessonsByCourseController = asyncHandler(async (req, res) => {
...
export const getLessonByIdController = asyncHandler(async (req, res) => {
    const lesson = await lessonService.getLessonById(req.params.id, req.user?.userId, req.user?.role);
    res.json({
        success: true,
        data: lesson,
    });
